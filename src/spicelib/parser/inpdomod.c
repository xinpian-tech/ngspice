...
char *INPdomodel(CKTcircuit *ckt, struct card *image, INPtables * tab)
{
    ...
    /*  --------  Check if model is a MOSFET --------- */
	       ...
	       || (strcmp(type_name, "psoi") == 0)) {
			...
			case 14: case 54:
			    err = INPfindVer(line, ver);
			    if ((strstr(ver, "default")) || (prefix("4.0", ver)) ||
			        (prefix("4.1", ver)) || (prefix("4.2", ver)) ||
			        (prefix("4.3", ver)) || (prefix("4.4", ver)) ||
			        (prefix("4.5", ver)) || (prefix("4.6", ver)) ||
			        (prefix("4.7", ver)) || (prefix("4.8", ver))) {
